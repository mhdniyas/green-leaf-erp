...
@php
    ...
    $unresolvedSyncFlagsCount = \App\Models\StaffSyncFlag::open()->count();

    if ($canViewStaffPayroll || $canViewStaffAdvancePayments) {
        $sidebarItems[] = [
            'label' => 'Staff Sync Flags'.($unresolvedSyncFlagsCount > 0 ? " ({$unresolvedSyncFlagsCount})" : ''),
            'href' => route('admin.staff.sync-flags.index'),
            'active' => request()->routeIs('admin.staff.sync-flags.*') || request()->is('admin/hr/staff-sync-flags*'),
