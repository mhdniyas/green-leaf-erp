...
@php
    ...
    if ($currentUser?->hasRole('admin') || $currentUser?->isMainAdmin() || \App\Services\Warehouse\WarehouseSalesAccessService::userCanMakeSales($currentUser)) {
        $salesItems[] = [
            'label' => 'Warehouse Sales',
            'href' => route('warehouse.sales.index'),
            'active' => request()->routeIs('warehouse.sales.*'),
    ...
    if (\App\Support\CashbookAccess::allows($currentUser, \App\Support\CashbookAccess::DashboardView) || \App\Support\CashbookAccess::canAccessCashbook($currentUser)) {
        $cashbookItems[] = [
            'label' => 'Dashboard',
            'href' => route('admin.cashbook.index'),
            'active' => request()->routeIs('admin.cashbook.index') || request()->routeIs('admin.cashbook.monthly-report.*'),
    ...
    if (\App\Support\CashbookAccess::allows($currentUser, \App\Support\CashbookAccess::ShopsView)) {
        $cashbookItems[] = [
            'label' => 'Shops',
            'href' => route('admin.cashbook.all-shops'),
            'active' => request()->routeIs('admin.cashbook.all-shops') || request()->routeIs('admin.cashbook.shop.*'),
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 21v-7.5a.75.75 0 0 1 .75-.75h3a.75.75 0 0 1 .75.75V21m-4.5 0H2.36m11.14 0H18m0 0h3.64m-1.39 0V9.349M3.75 21V9.349m0 0a3.001 3.001 0 0 0 3.75-.615A2.993 2.993 0 0 0 9.75 9.75c.896 0 1.7-.393 2.25-1.016a2.993 2.993 0 0 0 2.25 1.016c.896 0 1.7-.393 2.25-1.015a3.001 3.001 0 0 0 3.75.614m-16.5 0a3.004 3.004 0 0 1-.621-4.72l1.189-1.19A1.5 1.5 0 0 1 5.378 3h13.243a1.5 1.5 0 0 1 1.06.44l1.19 1.189a3 3 0 0 1-.621 4.72M6.75 18h3.75a.75.75 0 0 0 .75-.75V13.5a.75.75 0 0 0-.75-.75H6.75a.75.75 0 0 0-.75.75v3.75c0 .414.336.75.75.75Z" /></svg>',
        ];
    }

    if (\App\Support\CashbookAccess::allows($currentUser, \App\Support\CashbookAccess::MoneyFlowView)) {
        $cashbookItems[] = [
            'label' => 'Money Flow',
            'href' => route('admin.cashbook.money-flow'),
            'active' => request()->routeIs('admin.cashbook.money-flow*') || request()->routeIs('admin.cashbook.transaction.*'),
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" /></svg>',
        ];
    }

    if (\App\Support\CashbookAccess::allows($currentUser, \App\Support\CashbookAccess::AccountBalanceView)) {
        $cashbookItems[] = [
            'label' => 'Account Balance',
            'href' => route('admin.cashbook.account-balance'),
            'active' => request()->routeIs('admin.cashbook.account-balance*') || request()->routeIs('admin.cashbook.monthly-closing-summary.*') || request()->routeIs('admin.cashbook.bank-accounts.*'),
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6m-1.5 12V10.5M4.5 21V10.5" /></svg>',
        ];
    }

    if (\App\Support\CashbookAccess::allows($currentUser, \App\Support\CashbookAccess::ReconciliationView)) {
        $cashbookItems[] = [
            'label' => 'Reconciliation',
            'href' => route('admin.cashbook.finance.reconciliation'),
            'active' => request()->routeIs('admin.cashbook.finance*') && !request()->routeIs('admin.cashbook.finance.purchase*'),
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>',
        ];
    }

    if (\App\Support\CashbookAccess::allows($currentUser, \App\Support\CashbookAccess::InventoryView)) {
        $cashbookItems[] = [
            'label' => 'Inventory',
            'href' => route('admin.cashbook.inventory'),
            'active' => request()->routeIs('admin.cashbook.inventory*') || request()->routeIs('admin.cashbook.auto-match*') || request()->routeIs('admin.cashbook.bill-changes*') || request()->routeIs('admin.cashbook.purchaser-business-days.*'),
    ...
    if (\App\Support\CashbookAccess::allows($currentUser, \App\Support\CashbookAccess::ReportsView)) {
        $cashbookItems[] = [
            'label' => 'Reports',
            'href' => route('admin.cashbook.reports'),
            'active' => request()->routeIs('admin.cashbook.reports*') || request()->routeIs('admin.cashbook.warehouse-sales*'),
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>',
        ];
    }

    if (\App\Support\CashbookAccess::allows($currentUser, \App\Support\CashbookAccess::SettingsView)) {
        $cashbookItems[] = [
            'label' => 'Settings',
            'href' => route('admin.cashbook.settings'),
            'active' => request()->routeIs('admin.cashbook.settings*') || request()->routeIs('admin.cashbook.categories.*') || request()->routeIs('admin.cashbook.rules-config') || request()->routeIs('admin.cashbook.assets.*'),
    ...
    if (\App\Support\CashbookAccess::allows($currentUser, \App\Support\CashbookAccess::UserAccessManage)) {
        $cashbookItems[] = [
            'label' => 'User Access',
    ...
    if ($currentUser?->can('admin.user.view') || $currentUser?->hasRole('admin')) {
        ...
        $adminItems[] = [
            'label' => 'Zoho Books',
            'href' => route('admin.integrations.zoho-books.index'),
            'active' => request()->routeIs('admin.integrations.zoho-books.*'),
