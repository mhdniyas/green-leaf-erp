...
$modules = [
    ...
    [
        ...
        'badge'       => $salesStats && $salesStats['active_shops'] > 0 ? ($salesStats['active_shops'] . ' active') : null,
    ],
    [
...
<x-layouts.app title="Dashboard">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5 flex items-start gap-4">
            ...
            <div>
                <p class="text-xs text-gray-500 font-medium">Active Shops</p>
                <p class="text-2xl font-bold text-gray-900 mt-0.5">{{ $salesStats['active_shops'] }}</p>
                @can('sales.customer.view')
                <a href="{{ route('sales.customers.index') }}" class="text-xs text-blue-600 font-medium hover:underline mt-0.5 block">View shops →</a>
                @endcan
            </div>
        </div>
