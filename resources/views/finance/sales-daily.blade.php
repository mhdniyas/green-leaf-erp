<x-layouts.app title="Sales Daily Report">
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <section class="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs print:hidden">
            <x-export-toolbar
                excel-url="{{ route('finance.sales-daily.excel', ['date' => $date->format('Y-m-d'), 'status' => $statusFilter ?? 'all']) }}"
                pdf-url="{{ route('finance.sales-daily.pdf', ['date' => $date->format('Y-m-d'), 'status' => $statusFilter ?? 'all']) }}"
                title="Sales Daily Report ({{ $date->format('d M Y') }})"
                align="between"
            />
        </section>
