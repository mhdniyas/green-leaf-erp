<x-layouts.app title="Record Expense">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-xl font-bold text-gray-900 mt-4">Record New Expense</h1>
        <p class="text-sm text-gray-500 mt-0.5">Logs the transaction details and automatically posts debits/credits to the ledger.</p>
    </div>

    <div class="max-w-2xl bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <form method="POST" action="{{ route('finance.expenses.store') }}" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="number" name="amount" id="amount" step="0.01" min="0.01" value="{{ old('amount') }}" required placeholder="e.g. 5000.00"
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment_method" required
                        ...
                        <option value="cash" {{ old('payment_method') == 'cash' ? 'selected' : '' }}>Cash on Hand (Asset Account 1010)</option>
                        <option value="bank" {{ old('payment_method') == 'bank' ? 'selected' : '' }}>Bank Account (Asset Account 1020)</option>
                    </select>
                ...
                <div class="col-span-full">
                    ...
                    <input type="text" name="reference" id="reference" value="{{ old('reference') }}" placeholder="e.g. UT-MAY-2026, Rent-Inv-05"
                ...
                <div class="col-span-full">
                    ...
                    <textarea name="description" id="description" rows="3" placeholder="Describe the purpose of this expense..."
                              class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
