...
<x-layouts.app title="Vendor Reports">
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <section class="flex flex-wrap items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs print:hidden">
            <x-export-toolbar
                excel-url="{{ route('finance.vendors.excel', ['start_date' => $startDate->format('Y-m-d'), 'end_date' => $endDate->format('Y-m-d')]) }}"
                pdf-url="{{ route('finance.vendors.pdf', ['start_date' => $startDate->format('Y-m-d'), 'end_date' => $endDate->format('Y-m-d')]) }}"
