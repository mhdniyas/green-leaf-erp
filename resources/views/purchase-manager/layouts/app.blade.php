@php
    ...
    $sidebarItems = [
        [
            ...
            'active' => request()->routeIs('purchasing.dashboard') && ! request()->routeIs('purchasing.business-days.*') && ! request()->routeIs('purchaser.business-days.*'),
    ...
    if ($currentUser && ($currentUser->hasRole('admin') || $currentUser->hasRole('purchaser') || $currentUser->hasRole('purchase') || $currentUser->can('purchasing.order.view'))) {
        $sidebarItems[] = [
            'label' => 'Business Day',
            'href' => route('purchasing.business-days.index'),
            'active' => request()->routeIs('purchasing.business-days.*') || request()->routeIs('purchaser.business-days.*'),
            'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5" /></svg>',
            'children' => [
                [
                    'label' => 'Today',
                    'href' => route('purchasing.business-days.index'),
                    'active' => request()->routeIs('purchasing.business-days.show') || (request()->routeIs('purchasing.business-days.*') && ! request()->filled('view') && ! request()->boolean('history') && ! request()->filled('month')) || (request()->routeIs('purchaser.business-days.*') && ! request()->filled('view') && ! request()->boolean('history') && ! request()->filled('month')),
                ],
                [
                    'label' => 'Business Days',
                    'href' => route('purchasing.business-days.index', ['view' => 'history']),
                    'active' => (request()->routeIs('purchasing.business-days.index') || request()->routeIs('purchaser.business-days.index')) && (request()->input('view') === 'history' || request()->boolean('history') || request()->filled('month')),
                ],
            ],
        ];
    }
    ...
    $sidebarItems[] = [
        ...
        'active' => request()->routeIs('purchasing.prices.index'),
    ...
    $sidebarItems[] = [
        'label' => 'Daily Price Matrix',
        'href' => route('purchasing.prices.matrix.index', ['date' => $navDate]),
        'active' => request()->routeIs('purchasing.prices.matrix.*'),
        'icon' => '<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25ZM3.75 15.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25ZM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6ZM13.5 15.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25Z" /></svg>',
    ];
    $sidebarItems[] = [
    ...
    $sidebarItems[] = [
        ...
        'active' => request()->routeIs('purchasing.invoices.index') || (request()->routeIs('purchasing.invoices.*') && ! request()->routeIs('purchasing.invoices.flagged')),
    ...
    $sidebarItems[] = [
        'label' => 'Flagged Bills',
        'href' => route('purchasing.invoices.flagged'),
        'active' => request()->routeIs('purchasing.invoices.flagged'),
        'icon' => '<svg class="h-5 w-5 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>',
    ];

    if ($currentUser && (\App\Services\Warehouse\WarehouseSalesAccessService::userCanMakeSales($currentUser) || $currentUser->hasRole('admin') || $currentUser->isMainAdmin())) {
        $sidebarItems[] = [
