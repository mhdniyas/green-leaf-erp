...
@section('content')
    <div class="space-y-6">
            ...
            @foreach ($relationshipTypes as $type => $label)
                <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div class="mt-4 space-y-3">
                        @foreach ($groups->get($type, collect()) as $group)
                            <form method="POST" action="{{ route('purchasing.price-groups.update', $group) }}" class="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="relationship_type" value="{{ $type }}">
                                <div class="grid gap-3 sm:grid-cols-[1fr_120px]">
                                    <div>
                                        <label class="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Group</label>
                            ...
                            @if ($group->shops->isEmpty())
                                <form method="POST" action="{{ route('purchasing.price-groups.destroy', $group) }}">
                                    @csrf
                                    @method('DELETE')
        ...
        <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 class="text-lg font-black text-slate-950">Create Sub Price Group</h2>
            <form method="POST" action="{{ route('purchasing.price-groups.store') }}" class="mt-4 grid gap-3 md:grid-cols-[1fr_1fr_160px_auto] md:items-end">
                @csrf
                <div>
                    <label class="text-[11px] font-black uppercase tracking-[0.16em] text-slate-500">Main Category</label>
                    <select name="relationship_type" class="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-900">
                        @foreach ($relationshipTypes as $type => $label)
                ...
                <div>
                    <label class="text-[11px] font-black uppercase tracking-[0.16em] text-slate-500">Sub Group</label>
                ...
                <button class="rounded-2xl bg-cyan-500 px-5 py-3 text-sm font-black text-slate-950">Create Group</button>
            </form>
        </section>
                    ...
                    <table class="min-w-full text-left">
                        <thead class="border-b border-slate-200 bg-slate-50 text-[11px] font-black uppercase tracking-[0.16em] text-slate-500">
                            <tr>
                                ...
                                <th class="px-5 py-4">Price Group</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    ...
                                    <td class="px-5 py-4">
                                        <select name="shops[{{ $shop->id }}]" class="w-full max-w-sm rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-900">
                                            <option value="">Default Own / A</option>
                                            @foreach ($relationshipTypes as $type => $label)
