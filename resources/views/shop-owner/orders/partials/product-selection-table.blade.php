        ...
        @foreach ($allProductsForOrder as $productData)
            ...
            >
                <div class="flex items-start gap-3">
                    <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-[11px] font-black text-slate-600">
                        {{ $productData['sku'] }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <h4 class="truncate text-sm font-black text-slate-950">{{ $productData['name'] }}</h4>
                        <p class="mt-1 truncate text-xs font-semibold text-slate-500">{{ $productData['category'] }}</p>
                    </div>
                </div>

                <div class="mt-3 grid grid-cols-[minmax(0,1fr)_minmax(7rem,8.5rem)_auto] items-end gap-2">
                    <div>
                        <label class="mb-1 block text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Unit</label>
                        <div class="flex min-h-10 items-center rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-black uppercase text-slate-800">
                            {{ $productData['unit'] }}
                        </div>
                    </div>
                    <div>
                        <label for="order-qty-{{ $productData['id'] }}" class="mb-1 block text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Qty</label>
                        <input
                            ...
                            data-inline-qty
                            data-product-id="{{ $productData['id'] }}"
                            class="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-right text-base font-black text-slate-950 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                            placeholder="0"
                        >
                    </div>
                    <button type="button" data-next-product class="h-10 rounded-xl bg-slate-900 px-3 text-[11px] font-black uppercase tracking-[0.12em] text-white transition active:scale-95">
                        Next
                    </button>
                </div>

                <p data-row-selection-label class="{{ (float) $productData['current_qty'] > 0 ? '' : 'hidden' }} mt-2 text-[11px] font-bold text-emerald-700">
                    Selected for cart
                </p>
