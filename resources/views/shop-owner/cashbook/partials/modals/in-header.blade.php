...
<div id="in-header-modal" onclick="handleModalBackdropClick(event, 'in-header-modal')" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/40 backdrop-blur-xs hidden transition-all duration-200">
        ...
        <div class="space-y-1.5">
            @forelse($incomeHeaders as $hSec)
                ...
                <div onclick='selectHeaderForEntry(@json($hSec["id"]))'
                     class="p-2.5 sm:p-3 rounded-xl bg-slate-50/70 hover:bg-emerald-50/60 border border-slate-100 hover:border-emerald-200/80 transition cursor-pointer group flex items-center justify-between gap-2.5 active:scale-[0.99]">
                    <div class="flex items-center gap-2.5 min-w-0 flex-1">
                        <div class="w-7 h-7 rounded-full bg-white border border-slate-200/60 shadow-2xs flex items-center justify-center text-emerald-700 font-black text-xs uppercase shrink-0 group-hover:scale-105 transition">
                            {{ $firstLetter }}
                        </div>
                        <div class="min-w-0 flex-1">
                            <div class="text-xs sm:text-sm font-black text-slate-900 group-hover:text-emerald-800 transition truncate">
                                {{ $hSec['name'] }}
                            </div>
                            <div class="text-[10px] font-semibold text-slate-400 mt-0.5" id="in-modal-sub-{{ $hSec['id'] }}">
                                {{ count($hSec['settings']) }} categories
                            </div>
                        </div>
                    </div>
                    <div class="flex items-center gap-2 shrink-0">
                        <span class="font-mono text-xs font-black text-slate-900 bg-white px-2.5 py-0.5 rounded-full border border-slate-200/60 shadow-2xs" id="in-modal-total-{{ $hSec['id'] }}">
                            ₹0.00
                        </span>
                        <div class="w-6 h-6 rounded-full bg-white border border-slate-200/60 flex items-center justify-center text-slate-400 group-hover:text-emerald-700 transition">
