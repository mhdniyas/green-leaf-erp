@if ($entries->count() > 0)
            ...
            <tbody class="divide-y divide-slate-100">
                @foreach ($entries as $entry)
                    ...
                    <tr class="hover:bg-slate-50/40 transition-colors">
                        <td class="px-3 py-2.5 font-semibold text-slate-600 sm:px-4 sm:py-3">{{ $entry->business_date->format('d M Y') }}</td>
                        ...
                        <td class="px-3 py-2.5 text-right font-black text-slate-700 sm:px-4 sm:py-3">Rs. {{ number_format((float) $entry->opening_cash, 2) }}</td>
                        <td class="px-3 py-2.5 text-right font-black text-emerald-700 sm:px-4 sm:py-3">Rs. {{ number_format($income, 2) }}</td>
                        <td class="px-3 py-2.5 text-right font-black text-rose-700 sm:px-4 sm:py-3">Rs. {{ number_format($expense, 2) }}</td>
                        <td @class([
                            'px-3 py-2.5 text-right font-black sm:px-4 sm:py-3',
                            'text-emerald-700' => $net >= 0,
                            'text-rose-700' => $net < 0,
                        ])>
                            {{ $net >= 0 ? '+' : '-' }} Rs. {{ number_format(abs($net), 2) }}
                        </td>
                        <td class="px-3 py-2.5 text-right font-black text-slate-950 sm:px-4 sm:py-3">Rs. {{ number_format((float) $entry->closing_cash, 2) }}</td>
                    ...
                    @if ($entry->admin_note || $entry->shop_reply_note)
                        <tr class="bg-slate-50/50">
                            <td colspan="8" class="border-b border-slate-100 px-3 py-2 text-xs sm:px-4">
    ...
    <div class="mt-3 sm:mt-5">{{ $entries->links() }}</div>
@else
