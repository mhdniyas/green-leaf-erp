...
@section('content')
    <div class="mx-auto w-full max-w-2xl space-y-3">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-3.5 sm:p-4 shadow-xs" x-data="{
            ...
            <form method="POST" action="{{ $employee ? route('shop-owner.staff.employees.resubmit', $employee) : route('shop-owner.staff.employees.store') }}" class="space-y-3" onsubmit="const button = this.querySelector('[data-submit-button]'); button.disabled = true; button.classList.add('opacity-60', 'cursor-wait'); button.querySelector('[data-submit-label]').textContent = 'Submitting…';">
                @csrf
                @if($employee)
                    @method('PUT')
                @endif
                ...
                <!-- SUBMIT BUTTON -->
                <div class="pt-2">
                    <button type="submit" data-submit-button class="inline-flex h-11 w-full items-center justify-center gap-1.5 rounded-xl bg-emerald-600 text-xs font-black text-white shadow-xs hover:bg-emerald-700 active:scale-[0.99] transition disabled:pointer-events-none">
                        <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" />
                        </svg>
                        <span data-submit-label>{{ $employee ? 'Resubmit for HR Approval' : 'Submit for HR Approval' }}</span>
                    </button>
                </div>
            </form>
        </div>
