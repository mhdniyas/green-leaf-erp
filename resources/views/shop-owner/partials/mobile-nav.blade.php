@php
    $mobileNavItems = [
        ...
        ...((auth()->user()->shop?->isOwnedAccountingEnabled() ?? false) ? [[
            'label' => 'Accounting',
            'route' => 'shop-owner.accounting.index',
            'icon' => '<svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6M9 16h6M9 8h6M5 4h14a2 2 0 012 2v12a2 2 0 01-2 2H5a2 2 0 01-2-2V6a2 2 0 012-2z" /></svg>',
        ]] : []),
        [
            'label' => 'Finance',
    ...
    <nav class="mx-auto flex h-[60px] max-w-md items-center gap-1 rounded-[2rem] border border-slate-100 bg-white/96 px-2 shadow-[0_8px_40px_rgba(0,0,0,0.10),0_2px_12px_rgba(0,0,0,0.06)] backdrop-blur-xl">
        @foreach ($mobileNavItems as $item)
            @php
                ...
                if ($item['route'] === 'shop-owner.accounting.index') {
                    $isActive = request()->routeIs('shop-owner.accounting.*');
                }
            @endphp
