...
<div class="fixed inset-x-0 bottom-4 z-40 px-4 lg:hidden">
    <nav class="mx-auto max-w-md bg-white/95 backdrop-blur-md shadow-[0_-8px_30px_rgb(0,0,0,0.08),0_4px_20px_rgb(0,0,0,0.04)] border border-slate-100 rounded-[2rem] h-16 flex items-center justify-around px-4 relative">
                ...
                <div class="relative -mt-10">
                    <a
                        ...
                        @class([
                            ...
                            'bg-emerald-600 text-white hover:bg-emerald-700 hover:scale-105 active:scale-95 border-[6px] border-slate-100' => $isActive,
                            'bg-emerald-600 text-white/95 hover:bg-emerald-600 hover:scale-105 active:scale-95 border-[6px] border-slate-100' => !$isActive,
                        ])
                    >
                        {!! $item['icon'] !!}
                    </a>
                    @if ($isActive)
                        <div class="absolute -bottom-4 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-emerald-600 rounded-full"></div>
                    @endif
                </div>
            @else
                <a
                    href="{{ route($item['route']) }}"
                    @class([
                        'flex flex-col items-center justify-center w-12 h-12 rounded-2xl transition-all duration-200 relative',
                ...
                >
                    {!! $item['icon'] !!}
                    @if ($isActive)
                        <div class="absolute bottom-0 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-emerald-600 rounded-full"></div>
                    @endif
                </a>
            @endif
        @endforeach
