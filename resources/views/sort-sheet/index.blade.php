    ...
    <x-slot:actions>
        @if($hasMatrix)
            <div class="flex flex-wrap items-center gap-3">
                ...
                {{-- Subsection: Print Action & Shops to Print Filter --}}
                @if($canExport)
                <div class="flex flex-wrap items-center gap-1.5 rounded-2xl border border-slate-200 bg-slate-50/90 p-1.5 shadow-sm">
                    <span class="px-2 text-[10px] font-black uppercase tracking-wider text-slate-400">Print</span>

                    @if(isset($filteredShops) && $filteredShops->count() > 0)
                    {{-- Shops to Print Multi-Select Filter Dropdown --}}
                    <div class="relative" data-shops-to-print-root>
                        <button type="button"
                                class="shops-to-print-trigger inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 shadow-sm transition hover:bg-slate-50">
                            <svg class="w-3.5 h-3.5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 21v-7.5a.75.75 0 01.75-.75h3a.75.75 0 01.75.75V21m-4.5 0H2.25A2.25 2.25 0 010 18.75V5.25A2.25 2.25 0 012.25 3h19.5a2.25 2.25 0 012.25 2.25v13.5A2.25 2.25 0 0121.75 21H13.5z" />
                            </svg>
                            <span>Shops (<span class="shops-to-print-count">{{ $filteredShops->count() }}</span>/{{ $filteredShops->count() }})</span>
                            <span class="text-slate-400 text-[10px]">▾</span>
                        </button>
                        <div class="shops-to-print-panel absolute right-0 top-full z-40 mt-2 hidden w-72 rounded-2xl border border-slate-200 bg-white p-3 shadow-xl space-y-2.5">
                            <div class="flex items-center justify-between border-b border-slate-100 pb-2">
                                <span class="text-[10px] font-black uppercase tracking-wider text-slate-500">Shops to Print</span>
                                <div class="flex items-center gap-1">
                                    <button type="button" class="shops-to-print-select-all rounded px-2 py-0.5 text-[10px] font-bold text-emerald-700 hover:bg-emerald-50">Select All</button>
                                    <span class="text-slate-300">|</span>
                                    <button type="button" class="shops-to-print-clear-all rounded px-2 py-0.5 text-[10px] font-bold text-rose-700 hover:bg-rose-50">Clear All</button>
                                </div>
                            </div>
                            <div class="max-h-60 overflow-y-auto space-y-1 py-1">
                                @foreach($filteredShops as $shop)
                                    <label class="flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 cursor-pointer">
                                        <input type="checkbox" class="shop-to-print-cb rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" value="{{ $shop->id }}" checked>
                                        <span class="truncate">{{ $shop->name }}</span>
                                        @if($shop->warehouse_tag)
                                            <span class="ml-auto shrink-0 font-mono text-[9px] font-black bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded">{{ $shop->warehouse_tag }}</span>
                                        @endif
                                    </label>
                                @endforeach
                            </div>
                            <div class="shops-to-print-empty-msg hidden text-[11px] font-bold text-rose-600 bg-rose-50 p-2 rounded-lg border border-rose-100 text-center">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    @if($canExport)
                    @if(isset($filteredShops) && $filteredShops->count() > 0)
                    {{-- Table Header: Shops to Print Multi-Select Filter Dropdown --}}
                    <div class="relative" data-shops-to-print-root>
                        <button type="button"
                                class="shops-to-print-trigger inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[10px] font-bold text-slate-700 shadow-sm transition hover:bg-slate-50">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (!event.target.closest('[data-shops-to-print-root]')) {
            document.querySelectorAll('.shops-to-print-panel').forEach(p => p.classList.add('hidden'));
        }
    });

    // Shops to Print multi-select filter logic
    const shopsToPrintRoots = document.querySelectorAll('[data-shops-to-print-root]');
    if (shopsToPrintRoots.length > 0) {
        function syncShopsToPrint(changedCheckbox) {
            const val = changedCheckbox.value;
            const checked = changedCheckbox.checked;

            document.querySelectorAll(`.shop-to-print-cb[value="${val}"]`).forEach(cb => {
                cb.checked = checked;
            });

            updateShopsToPrintState();
        }

        function updateShopsToPrintState() {
            const firstPanel = document.querySelector('.shops-to-print-panel');
            if (!firstPanel) return;

            const allCbs = Array.from(firstPanel.querySelectorAll('.shop-to-print-cb'));
            const totalCount = allCbs.length;
            const checkedCbs = allCbs.filter(cb => cb.checked);
            const checkedCount = checkedCbs.length;
            const checkedIds = checkedCbs.map(cb => cb.value);

            document.querySelectorAll('.shops-to-print-count').forEach(el => {
                el.textContent = checkedCount;
            });

            document.querySelectorAll('.shops-to-print-empty-msg').forEach(el => {
                if (checkedCount === 0) {
            ...
            const printLinks = document.querySelectorAll('#print-sort-sheet-btn, #export-pdf-btn, #export-excel-btn, #print-selection-btn, #print-portrait-btn, #print-wide-btn, #print-grid-btn');
            ...
            printLinks.forEach(link => {
                ...
                if (checkedCount === 0) {
                    ...
                    link.title = 'Select at least one shop.';
                } else {
                    ...
                    try {
                        const baseUrl = link.dataset.originalHref;
                        const url = new URL(baseUrl, window.location.origin);
                        url.searchParams.delete('print_shop_ids[]');
                        url.searchParams.delete('shop_ids[]');

                        if (checkedCount < totalCount) {
                            checkedIds.forEach(id => {
                                url.searchParams.append('print_shop_ids[]', id);
                            });
                        }
                        link.setAttribute('href', url.pathname + url.search);
                    } catch (e) {
                        console.error('Error updating print link URL:', e);
                    }
                }
            });
        }

        shopsToPrintRoots.forEach(root => {
            const trigger = root.querySelector('.shops-to-print-trigger');
            const panel = root.querySelector('.shops-to-print-panel');
            const selectAllBtn = root.querySelector('.shops-to-print-select-all');
            const clearAllBtn = root.querySelector('.shops-to-print-clear-all');
            ...
            if (selectAllBtn) {
                selectAllBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    document.querySelectorAll('.shop-to-print-cb').forEach(cb => cb.checked = true);
                    updateShopsToPrintState();
                });
            }
            ...
            if (clearAllBtn) {
                clearAllBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    document.querySelectorAll('.shop-to-print-cb').forEach(cb => cb.checked = false);
                    updateShopsToPrintState();
                });
            }
        });

        document.querySelectorAll('.shop-to-print-cb').forEach(cb => {
            cb.addEventListener('change', () => syncShopsToPrint(cb));
        });
