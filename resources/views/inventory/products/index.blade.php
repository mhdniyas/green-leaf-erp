<x-layouts.inventory title="Products">
    ...
    <div class="mb-6 border-b border-gray-200">
        <nav class="-mb-px flex space-x-6">
            <a href="{{ route('inventory.products.index') }}"
               class="{{ !request('status') ? 'border-brand-500 text-brand-600 font-semibold' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 font-medium' }} whitespace-nowrap pb-3 px-1 border-b-2 text-sm">
                All Products
            </a>
            <a href="{{ route('inventory.products.index', array_merge(request()->except('page'), ['status' => 'active'])) }}"
               class="{{ request('status') === 'active' ? 'border-brand-500 text-brand-600 font-semibold' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 font-medium' }} whitespace-nowrap pb-3 px-1 border-b-2 text-sm">
                Active
            </a>
            <a href="{{ route('inventory.products.index', array_merge(request()->except('page'), ['status' => 'inactive'])) }}"
               class="{{ request('status') === 'inactive' ? 'border-brand-500 text-brand-600 font-semibold' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 font-medium' }} whitespace-nowrap pb-3 px-1 border-b-2 text-sm">
                Inactive
            </a>
            @if(auth()->user()?->hasRole('admin'))
                <a href="{{ route('inventory.products.trash') }}"
                   class="border-transparent text-gray-500 hover:text-red-600 hover:border-red-300 whitespace-nowrap pb-3 px-1 border-b-2 font-medium text-sm flex items-center gap-2">
                    Deleted Products
                    <span class="rounded-full {{ ($deletedCount ?? 0) > 0 ? 'bg-red-100 text-red-700 font-bold' : 'bg-gray-100 text-gray-600 font-medium' }} px-2 py-0.5 text-xs">
                        {{ $deletedCount ?? 0 }}
                    </span>
                </a>
                <a href="{{ route('inventory.products.flags') }}"
                   class="border-transparent text-gray-500 hover:text-orange-600 hover:border-orange-300 whitespace-nowrap pb-3 px-1 border-b-2 font-medium text-sm flex items-center gap-1.5">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" x2="4" y1="22" y2="15"/></svg>
                    Flags
                </a>
            @endif
        </nav>
    ...
    <form method="GET" action="{{ route('inventory.products.index') }}" class="flex flex-wrap items-center gap-3 mb-6">
        <div class="relative flex-1 min-w-48">
            ...
            <input id="search-input" type="text" name="search" value="{{ request('search') }}" placeholder="Search by name, SKU or unit…"
