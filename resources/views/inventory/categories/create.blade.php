<x-layouts.inventory title="Add Category">

    <div class="max-w-2xl">
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-100">
                <h2 class="text-sm font-semibold text-gray-900">New Product Category</h2>
                <p class="text-xs text-gray-500 mt-0.5">Add a new category to group related products.</p>
            </div>

            <form method="POST" action="{{ route('inventory.categories.store') }}" class="p-6 space-y-5">
                @csrf
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea id="description" name="description" rows="4"
                              ...
                              class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 @error('description') border-red-300 @enderror">{{ old('description') }}</textarea>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                    <div class="flex items-center gap-3">
                        ...
                        <label for="is_active" class="text-sm font-semibold text-slate-800 cursor-pointer">Active</label>
                    </div>
