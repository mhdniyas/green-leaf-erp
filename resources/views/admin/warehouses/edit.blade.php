    ...
    <div class="max-w-7xl">
        <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-xs">
            ...
            <form method="POST" action="{{ route('admin.warehouses.update', $warehouse) }}" class="space-y-4">
                ...
                @php
                    $selectedCategoryIds = collect(old('category_ids', $warehouse->categories->pluck('id')->map(fn ($id) => (string) $id)->all()))
                        ->map(fn ($id) => (string) $id)
                        ->all();
                @endphp
                <div class="pt-2">
                    <div class="flex items-center justify-between gap-3 mb-2">
                        ...
                        <span class="text-[10px] font-bold text-gray-400">{{ count($selectedCategoryIds) }} selected</span>
                    </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <div class="flex items-center justify-between gap-3">
                        <h3 class="text-xs font-black uppercase tracking-[0.16em] text-slate-500">Current Default Products</h3>
                        <span class="text-xs font-bold text-slate-500">{{ $defaultProducts->count() }} products</span>
                    </div>
                    <div class="mt-3 grid max-h-72 grid-cols-1 gap-2 overflow-y-auto sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        @forelse($defaultProducts as $product)
                            <article class="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                                <div class="min-w-0">
                                    <h4 class="truncate text-sm font-black text-slate-950">{{ $product->name }}</h4>
                        ...
                        @empty
                            <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-8 text-center text-sm font-bold text-slate-500">
                                No products use this as default warehouse.
                            </div>
                        @endforelse
                    </div>
                </div>
                ...
                @php
                    $selectedProductIds = collect(old('product_ids', $defaultProducts->pluck('id')->map(fn ($id) => (string) $id)->all()))
                        ->map(fn ($id) => (string) $id)
                        ->all();
                @endphp
                <div class="pt-2">
