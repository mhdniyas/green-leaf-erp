<x-layouts.admin title="Edit User">
    ...
    <form method="POST" action="{{ route('admin.users.update', $user) }}" class="mx-auto max-w-5xl space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="rounded-lg border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-100 px-5 py-4">
                ...
                <p class="mt-1 text-xs text-gray-500">Update login details, shop assignment, and role access for {{ $user->name }}.</p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 p-5 md:grid-cols-2">
                ...
                <div>
                    <label for="password" class="mb-1.5 block text-xs font-semibold text-gray-700">Password</label>
        ...
        @php
            ...
            $savedDefaultWarehouseId = $user->warehouses->first(fn ($warehouse) => (bool) $warehouse->pivot->is_default)?->id;
        @endphp
        <div class="rounded-lg border border-gray-200 bg-white shadow-sm">
            <div class="border-b border-gray-100 px-5 py-4">
            ...
            <div class="space-y-4 p-5">
                    ...
                    @foreach($warehouses as $warehouse)
                        <label class="flex cursor-pointer items-center gap-3 rounded-lg border border-gray-200 p-4 hover:border-brand-200 hover:bg-brand-50/40">
                            <input type="checkbox" name="warehouse_ids[]" value="{{ $warehouse->id }}"
                                   @checked(in_array($warehouse->id, array_map('intval', old('warehouse_ids', $assignedWarehouseIds)), true))
                ...
                <div class="max-w-md">
                    ...
                    <select name="default_warehouse_id" id="default_warehouse_id"
                        ...
                        @foreach($warehouses as $warehouse)
                            <option value="{{ $warehouse->id }}" @selected((string) old('default_warehouse_id', $savedDefaultWarehouseId) === (string) $warehouse->id)>{{ $warehouse->name }}</option>
                        @endforeach
                    </select>
