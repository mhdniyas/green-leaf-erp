...
@section('title', 'Edit Product Filter - Purchase Cashbook')
@section('header_title')
    <i data-lucide="edit-3" class="h-5 w-5 text-emerald-600"></i> Edit Product Filter
@endsection

@section('header_subtitle')
    Update product membership for {{ $productFilter->name }}
@endsection
...
<div class="mx-auto max-w-[96rem] space-y-5" x-data="productFilterManager()">
    <div class="flex items-center justify-between border-b border-slate-200 pb-4">
        <div>
            <h1 class="text-xl font-black text-slate-950">Edit Filter: {{ $productFilter->name }}</h1>
            <p class="mt-0.5 text-xs font-semibold text-slate-500">Modify the filter name or check/uncheck products to update this saved filter.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.cashbook.finance.purchase.product-filters.update', $productFilter->uuid) }}" @submit="validateForm($event)" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm space-y-4">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $productFilter->name) }}" placeholder="e.g. Local Vegetables, Morning Market, Fruit Selection..." required class="mt-1.5 min-h-10 w-full max-w-lg rounded-lg border border-slate-300 px-3 text-xs font-bold text-slate-900 focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600">
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm space-y-4">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-3">
                <div>
                    ...
                    <p class="text-[11px] font-semibold text-slate-500">Click a category button to toggle all its products, or pick individual items below.</p>
                </div>
        ...
        <div class="sticky bottom-0 rounded-lg border border-slate-200 bg-white/95 backdrop-blur p-4 shadow-lg flex items-center justify-between z-10">
            ...
            <div class="flex items-center gap-2">
                ...
                <button type="submit" class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-700 px-5 py-2 text-xs font-black text-white hover:bg-emerald-800 shadow-sm">
                    <i data-lucide="check" class="h-4 w-4"></i> Update Product Filter
                </button>
            </div>
        </div>
    </form>
...
function productFilterManager() {
    return {
        selectedProducts: @json(old('product_ids', $selectedProductIds)),
        searchQuery: '',
