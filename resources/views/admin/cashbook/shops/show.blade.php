                ...
                @else
                    <div class="space-y-3">
                        @foreach($dailySettlement['collections'] as $col)
                            <div class="p-4 rounded-2xl border border-slate-100 bg-slate-50/60 hover:bg-slate-50 transition-all flex flex-col gap-3">
                                ...
                                @if(!empty($col['has_bank_adjustment_rules']))
                                    <div class="mt-1 pt-3 border-t border-slate-200/80" x-data="{
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center gap-2">
                                                <span class="text-[11px] font-extrabold uppercase tracking-wider text-slate-500">Bank Settlement</span>
                                                @if(($col['adjustment_total'] ?? 0) != 0)
                                                    <span class="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full {{ ($col['adjustment_total'] ?? 0) < 0 ? 'bg-rose-50 text-rose-700 border border-rose-200' : 'bg-emerald-50 text-emerald-700 border border-emerald-200' }}">
                                                        {{ ($col['adjustment_total'] ?? 0) > 0 ? '+' : '' }}₹{{ number_format($col['adjustment_total'], 2) }}
                                                    </span>
                                                @endif
                                            </div>
                                            <button type="button" @click="open = !open" class="text-xs font-bold text-slate-600 hover:text-slate-900 inline-flex items-center gap-1">
                                                <span x-text="open ? 'Close' : 'Adjust / Edit'"></span>
                                                <i data-lucide="chevron-down" class="w-3.5 h-3.5 transition-transform" :class="open ? 'rotate-180' : ''"></i>
                                            </button>
                                        </div>

                                        <div x-show="open" x-cloak class="mt-3 space-y-3 bg-white p-3.5 rounded-xl border border-slate-200/90 shadow-xs">
                                            <template x-for="(adj, idx) in adjustments" :key="adj.rule_id">
                                                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-2.5 last:border-0 last:pb-0">
                                                    <div class="flex items-center gap-2">
                                                        <span class="text-xs font-bold text-slate-800" x-text="adj.label"></span>
                                                        <span class="text-[9px] font-black uppercase px-1.5 py-0.5 rounded"
                                                              :class="adj.direction === 'plus' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'"
                                                              x-text="adj.direction.toUpperCase()"></span>
                                                    </div>
                                                    <div class="flex items-center gap-2 w-full sm:w-auto">
                                                        <div class="relative flex-1 sm:w-32">
                                                            ...
                                                            <input type="number" step="0.01" min="0" x-model="adj.amount" placeholder="0.00"
                                                                   class="w-full pl-6 pr-2 py-1.5 text-xs font-mono font-bold text-slate-900 border border-slate-200 rounded-lg focus:outline-none focus:border-slate-400">
                                                        </div>
                                                    </div>
                                                </div>
                                            </template>
                                            ...
                                            <div class="pt-2 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                                <div class="text-xs">
                                                    <span class="text-slate-500 font-bold">Expected Bank: </span>
                                                    <strong class="font-mono font-black text-slate-950 text-sm" x-text="'₹' + Number(expectedAmount).toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2})"></strong>
                                                </div>
                                                <button type="button" @click="saveAdjustments()" :disabled="saving"
                                                        class="px-4 py-1.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-black rounded-lg transition disabled:bg-slate-300">
                                                    <span x-text="saving ? 'Saving...' : 'Save Adjustments'"></span>
                                                </button>
                                            </div>
                                        </div>

                                        <div x-show="!open" class="mt-1 flex items-center justify-between text-xs">
                                            <span class="text-slate-500 font-semibold">Expected Bank:</span>
                                            <span class="font-mono font-black text-slate-900">₹{{ number_format($col['expected_bank_amount'] ?? $col['amount'], 2) }}</span>
                                        </div>
                                    </div>
                                @endif
                            </div>
