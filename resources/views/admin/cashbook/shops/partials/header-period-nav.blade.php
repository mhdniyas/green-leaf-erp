...
<header class="rounded-3xl border border-slate-200/80 bg-white p-4 sm:p-6 shadow-xs space-y-5" aria-label="Shop Cashbook Header and Period Navigation">
        ...
        <div>
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <span class="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-bold text-slate-700">
                    {{ request()->routeIs('admin.cashbook.shop.overview') ? 'Cashbook Overview' : 'Sales Report' }}
                </span>
            </div>
            <p class="mt-1 text-xs font-semibold text-slate-500">
                ...
                @else
                    Sales Report &bull; Monthly shop sales and expense summary
                @endif
            </p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-100 bg-slate-50/80 p-3 sm:p-4" x-data="{ activeMode: '{{ $periodMode }}' }">
        @php
            $navRouteName = request()->routeIs('admin.cashbook.shop.overview') ? 'admin.cashbook.shop.overview' : 'admin.cashbook.shop.show';
        @endphp
        <form method="GET" action="{{ route($navRouteName, $currentShopSlugOrId) }}" class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <input type="hidden" name="month" value="{{ $month }}">
            <input type="hidden" name="period_mode" :value="activeMode">

            <!-- Mode Switcher Buttons & Quick Time Shortcuts -->
            <div class="flex items-center gap-1.5 bg-slate-200/70 p-1 rounded-xl w-fit flex-wrap">
                ...
                @php
                    $todayDateStr = \Illuminate\Support\Carbon::today()->toDateString();
                    $todayMonthStr = \Illuminate\Support\Carbon::today()->format('Y-m');
                    $todayMonthStart = \Illuminate\Support\Carbon::today()->startOfMonth()->toDateString();

                    $yesterdayDateStr = \Illuminate\Support\Carbon::yesterday()->toDateString();
                    $yesterdayMonthStr = \Illuminate\Support\Carbon::yesterday()->format('Y-m');
                    $yesterdayMonthStart = \Illuminate\Support\Carbon::yesterday()->startOfMonth()->toDateString();

                    $isTodayActive = $periodMode === 'day' && $periodStart === $todayDateStr;
                    $isYesterdayActive = $periodMode === 'day' && $periodStart === $yesterdayDateStr;
                    $isUptoTodayActive = $periodMode === 'custom' && $periodStart === $todayMonthStart && $periodEnd === $todayDateStr;
                    $isUptoYesterdayActive = $periodMode === 'custom' && $periodStart === $yesterdayMonthStart && $periodEnd === $yesterdayDateStr;
                @endphp

                <a href="{{ route($navRouteName, ['shop' => $currentShopSlugOrId, 'period_mode' => 'day', 'date' => $todayDateStr, 'month' => $todayMonthStr]) }}"
                   class="px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer {{ $isTodayActive ? 'bg-emerald-600 text-white shadow-xs' : 'text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200/60' }}">
                    Today
                </a>

                <a href="{{ route($navRouteName, ['shop' => $currentShopSlugOrId, 'period_mode' => 'day', 'date' => $yesterdayDateStr, 'month' => $yesterdayMonthStr]) }}"
                   class="px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer {{ $isYesterdayActive ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-700 bg-white hover:bg-slate-100 border border-slate-200/80' }}">
                    Yesterday
                </a>

                <a href="{{ route($navRouteName, ['shop' => $currentShopSlugOrId, 'period_mode' => 'custom', 'from' => $todayMonthStart, 'to' => $todayDateStr, 'month' => $todayMonthStr]) }}"
                   class="px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer {{ $isUptoTodayActive ? 'bg-teal-700 text-white shadow-xs' : 'text-teal-900 bg-teal-50 hover:bg-teal-100 border border-teal-200/80' }}">
                    Up to Today
                </a>

                <a href="{{ route($navRouteName, ['shop' => $currentShopSlugOrId, 'period_mode' => 'custom', 'from' => $yesterdayMonthStart, 'to' => $yesterdayDateStr, 'month' => $yesterdayMonthStr]) }}"
                   class="px-3 py-1.5 rounded-lg text-xs font-extrabold transition cursor-pointer {{ $isUptoYesterdayActive ? 'bg-purple-700 text-white shadow-xs' : 'text-purple-900 bg-purple-50 hover:bg-purple-100 border border-purple-200/80' }}">
                    Up to Yesterday
                </a>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center gap-1">
                    <a href="{{ route($navRouteName, ['shop' => $currentShopSlugOrId, 'month' => $prevMonth, 'period_mode' => 'month']) }}"
                    ...
                    <a href="{{ route($navRouteName, ['shop' => $currentShopSlugOrId, 'month' => $nextMonth, 'period_mode' => 'month']) }}"
