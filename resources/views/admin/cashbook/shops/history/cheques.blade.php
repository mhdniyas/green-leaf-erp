...
@section('title', ($currentShop->name ?: 'Shop').' — Cheques Full History')
...
<div class="mx-auto max-w-7xl space-y-6 pb-16">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Shop Cheque Receipts &amp; Floating Status History</p>
            </div>
        </div>
    ...
    <!-- Filter Bar -->
    <form method="GET" action="{{ route('admin.cashbook.shop.history.cheques', $currentShop->slug ?: $currentShop->shop_id) }}"
        ...
        <div class="flex items-center gap-1.5">
            ...
            <select name="status" class="px-2.5 py-1.5 bg-slate-50 rounded-xl border border-slate-300 text-slate-900 focus:bg-white focus:outline-none">
                <option value="">All Statuses</option>
                <option value="pending" {{ $status === 'pending' || $status === 'floating' ? 'selected' : '' }}>Floating / Pending</option>
        ...
        <div class="flex items-center gap-1.5 flex-1 min-w-[200px]">
            <input type="text" name="search" value="{{ $search }}" placeholder="Search cheque number, bank name..." class="w-full px-3 py-1.5 bg-slate-50 rounded-xl border border-slate-300 text-slate-900 focus:bg-white focus:outline-none">
        </div>
        ...
        @if($status || $search)
            <a href="{{ route('admin.cashbook.shop.history.cheques', $currentShop->slug ?: $currentShop->shop_id) }}" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-bold transition">
                Reset
            </a>
        @endif
    </form>
    ...
    <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            <h2 class="text-sm font-black uppercase tracking-wider text-slate-900">All Cheque Records</h2>
            <span class="text-xs text-slate-500 font-mono font-bold">Total: {{ $cheques->total() }} records</span>
        </div>

        @include('admin.cashbook.shops.partials.cheques-table', ['cheques' => $cheques, 'isFull' => true])
