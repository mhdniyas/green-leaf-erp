...
<div class="rounded-2xl border border-slate-200 bg-white p-3 sm:p-4 shadow-xs">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div class="flex items-center gap-2">
            ...
            <span class="rounded-md bg-slate-100 px-2 py-0.5 font-mono text-[10px] font-bold text-slate-500">{{ $currentShop->code ?: 'SHOP-'.($currentShop->shop_id ?? $currentShop->id) }}</span>
        </div>
        <div class="inline-flex items-center gap-1 rounded-xl bg-slate-100/90 p-1 border border-slate-200/80 overflow-x-auto max-w-full">
            ...
            <a href="{{ route('admin.cashbook.settings.shop.vendors.index', $resolvedShopKey) }}"
               class="px-3 py-1.5 rounded-lg text-xs font-bold transition whitespace-nowrap {{ $activeTab === 'vendors' ? 'bg-white text-slate-950 shadow-2xs font-extrabold' : 'text-slate-600 hover:text-slate-950' }}">
                Vendors
            </a>
