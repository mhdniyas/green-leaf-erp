    ...
    <form method="GET" action="{{ route('admin.cashbook.reports') }}" class="grid gap-3 md:grid-cols-[1fr_auto] md:items-end">
        <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <label class="space-y-1">
                ...
                <select name="timeframe" class="h-11 w-full rounded-2xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-800">
            ...
            <label class="space-y-1">
                ...
                <input type="date" name="date" value="{{ $selectedDate }}" class="h-11 w-full rounded-2xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-800">
            </label>
            <label class="space-y-1">
                <span class="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">From</span>
                <input type="date" name="start_date" value="{{ $startDate }}" class="h-11 w-full rounded-2xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-800">
            </label>
            <label class="space-y-1">
                <span class="text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">To</span>
                <input type="date" name="end_date" value="{{ $endDate }}" class="h-11 w-full rounded-2xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-800">
            </label>
        </div>
