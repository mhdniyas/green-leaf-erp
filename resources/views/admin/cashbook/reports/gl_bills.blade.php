...
@section('content')
    <div x-data="{ activeInvoiceId: null }" class="mx-auto max-w-4xl space-y-4">
        <!-- Top Fintech Header -->
        <div class="flex items-center justify-between pt-1">
            ...
            <form method="GET" action="{{ route('admin.cashbook.reports.gl-bills') }}" class="flex items-center gap-2">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            <div class="inline-flex items-center max-w-full overflow-x-auto rounded-2xl bg-slate-200/70 p-1 shrink-0 gap-0.5">
                <a href="{{ route('admin.cashbook.reports.gl-bills', ['timeframe' => 'today', 'shop_id' => $selectedShopId]) }}" class="rounded-xl px-3 py-1 text-xs font-extrabold transition-all {{ $timeframe === 'today' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600 hover:text-slate-900' }}">Today</a>
                <a href="{{ route('admin.cashbook.reports.gl-bills', ['timeframe' => 'weekly', 'shop_id' => $selectedShopId]) }}" class="rounded-xl px-3 py-1 text-xs font-extrabold transition-all {{ $timeframe === 'weekly' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600 hover:text-slate-900' }}">Week</a>
                <a href="{{ route('admin.cashbook.reports.gl-bills', ['timeframe' => 'monthly', 'shop_id' => $selectedShopId]) }}" class="rounded-xl px-3 py-1 text-xs font-extrabold transition-all {{ $timeframe === 'monthly' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-600 hover:text-slate-900' }}">Month</a>
            ...
            <!-- Custom Filter Form -->
            <form method="GET" action="{{ route('admin.cashbook.reports.gl-bills') }}" class="flex flex-wrap sm:flex-nowrap items-center gap-1.5">
        ...
        <div class="rounded-[28px] border border-slate-100 bg-white p-4 sm:p-5 shadow-[0_8px_30px_rgba(0,0,0,0.04)] space-y-4">
            <div class="flex items-center justify-between px-1">
                <div>
                    ...
                    <p class="text-[10px] font-semibold text-slate-400">Click any bill row to view full invoice receipt</p>
                </div>
                        ...
                        @forelse ($invoices as $inv)
                            <tr @click="activeInvoiceId = {{ $inv->id }}" class="hover:bg-slate-50/80 transition cursor-pointer group">
                                ...
                                <td class="py-3 px-3 text-right">
                                    ...
                                    @php($bal = (float) $inv->balance_amount)
                                    @php($paid = (float) $inv->paid_amount)
                                    @if ($bal <= 0 && $paid > 0)
                                        <span class="inline-block rounded-full bg-emerald-50 px-1.5 py-0.5 text-[8px] font-black uppercase text-emerald-700">Paid</span>
                                    @elseif ($paid > 0)
                                        <span class="inline-block rounded-full bg-amber-50 px-1.5 py-0.5 text-[8px] font-black uppercase text-amber-700">Partial</span>
                                    @else
                                        <span class="inline-block rounded-full bg-rose-50 px-1.5 py-0.5 text-[8px] font-black uppercase text-rose-700">Unpaid</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="3" class="py-8 text-center text-xs font-bold text-slate-400">
                                    No GL invoices or delivery bills found for the selected period.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
