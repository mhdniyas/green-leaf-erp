                ...
                <div x-data="{ open: false }" class="relative inline-block text-left">
                    <button @click="open = !open" type="button" class="inline-flex items-center justify-between gap-2 h-10 px-3.5 rounded-2xl bg-white border border-slate-200/90 text-xs font-black text-slate-800 shadow-xs hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-900/10 transition-all cursor-pointer">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="max-w-[130px] truncate">{{ $selectedShop ? ($selectedShop->name ?: 'Shop #'.$selectedShop->shop_id) : 'Select Outlet...' }}</span>
                        </div>
                        <i data-lucide="chevron-down" class="w-3.5 h-3.5 text-slate-400 transition-transform duration-200" :class="{ 'rotate-180': open }"></i>
                    </button>
                    ...
                    <div x-show="open"
                        ...
                        <div class="py-1 space-y-0.5">
                            @foreach ($shops as $s)
                                <a href="{{ route('admin.cashbook.reports.gl-bills', ['timeframe' => $timeframe, 'shop_id' => $s->shop_id, 'product_filter' => $selectedProductFilterUuid, 'start_date' => $startDate, 'end_date' => $endDate]) }}"
                                   @click="loading = true"
                                ...
                                <tr>
                                    <td colspan="3" class="py-8 text-center text-xs font-bold text-slate-400">
                                        @if ($selectedProductFilter)
                                            No GL invoices matching product filter "{{ $selectedProductFilter->name }}" found for {{ $selectedShop?->name ?? 'the selected outlet' }} in the selected period.
                                        @else
                                            No GL invoices or delivery bills found for {{ $selectedShop?->name ?? 'the selected outlet' }} in the selected period.
                                        @endif
                                    </td>
                                </tr>
                            @endforelse
