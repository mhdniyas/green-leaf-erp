<x-layouts.staff title="Staff Attendance Board">
    <div class="mx-auto max-w-[1600px] space-y-5">
                    ...
                    <div>
                        ...
                        <select id="attendance-modal-employee-id" name="employee_id" class="w-full rounded-xl border border-slate-200 p-2.5 text-xs font-bold text-slate-900 focus:border-emerald-600 focus:ring-emerald-600" required>
                            ...
                            @foreach($allActiveEmployees as $empOpt)
                                <option value="{{ $empOpt->id }}">{{ $empOpt->name }} {{ $empOpt->defaultShop ? '· '.$empOpt->defaultShop->name : '' }}</option>
                            @endforeach
                        </select>
                    </div>
        ...
        <div id="admin-attendance-details-modal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-3" role="dialog" aria-modal="true">
            <div id="admin-attendance-details-backdrop" class="fixed inset-0 bg-slate-950/60 backdrop-blur-xs"></div>
            <div class="relative w-full max-w-xs sm:max-w-sm rounded-xl bg-white p-3.5 shadow-xl border border-slate-200 space-y-2.5 z-10 text-xs">
                <div class="flex items-center justify-between border-b border-slate-100 pb-2">
                    ...
                    <button type="button" id="btn-close-details-modal" class="text-slate-400 hover:text-slate-700 text-xs font-bold cursor-pointer p-0.5">✕</button>
                </div>

                <!-- PROFILE HEADER -->
                <div class="flex items-center gap-2.5">
                    <div id="details-modal-avatar-container" class="shrink-0">
                        <div id="details-modal-initials" class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 text-xs font-bold text-white"></div>
                        <img id="details-modal-photo" src="" class="hidden h-8 w-8 rounded-full object-cover border border-slate-200" alt="">
                    </div>
                    <div class="min-w-0">
                        <h4 id="details-modal-name" class="text-xs font-black text-slate-950 truncate leading-tight"></h4>
                        <p id="details-modal-meta" class="text-[10px] font-semibold text-slate-400 truncate leading-tight"></p>
                    </div>
                </div>
                ...
                <div class="space-y-1.5 text-[11px]">
                    <div class="rounded-lg border border-slate-100 bg-slate-50 px-2 py-1">
                        ...
                        <p id="details-modal-shop" class="font-bold text-slate-900 leading-tight">—</p>
                    </div>

                    <div class="grid grid-cols-2 gap-1.5">
                        <div class="rounded-lg border border-slate-100 bg-slate-50 px-2 py-1">
                            <p class="text-[8px] font-bold text-slate-400 uppercase">Status</p>
                            <p id="details-modal-status" class="font-bold text-slate-900 leading-tight">—</p>
                        </div>
                        <div class="rounded-lg border border-slate-100 bg-slate-50 px-2 py-1">
                            <p class="text-[8px] font-bold text-slate-400 uppercase">Marked Time</p>
                            <p id="details-modal-marked-at" class="font-bold text-slate-900 leading-tight">—</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-1.5">
                        <div class="rounded-lg border border-slate-100 bg-slate-50 px-2 py-1">
                            ...
                            <p id="details-modal-phone" class="font-semibold text-slate-800 leading-tight truncate">—</p>
                        </div>
                        <div class="rounded-lg border border-slate-100 bg-slate-50 px-2 py-1">
                            <p class="text-[8px] font-bold text-slate-400 uppercase">Emergency Contact</p>
                            <p id="details-modal-emergency" class="font-semibold text-slate-800 leading-tight truncate">—</p>
                        </div>
                    </div>

                    <div class="rounded-lg border border-slate-100 bg-slate-50 px-2 py-1">
                        <p class="text-[8px] font-bold text-slate-400 uppercase">Marked By & Source</p>
                        <p id="details-modal-marked-by" class="font-semibold text-slate-800 leading-tight">—</p>
                    </div>

                    <div id="details-modal-notes-container" class="hidden rounded-lg border border-slate-100 bg-slate-50 px-2 py-1">
                        <p class="text-[8px] font-bold text-slate-400 uppercase">Reason / Note</p>
                        <p id="details-modal-notes" class="font-semibold text-slate-700 whitespace-pre-line leading-tight text-[10px]"></p>
                    </div>
                </div>

                <!-- FOOTER ACTIONS -->
                <div class="flex items-center justify-between pt-1.5 border-t border-slate-100">
                    <a id="details-modal-calendar-link" href="#" class="rounded-lg border border-slate-200 px-2.5 py-1 text-[10px] font-bold text-slate-700 hover:bg-slate-100">
                        View Calendar
                    </a>
                    <button type="button" id="btn-cancel-details-modal" class="rounded-lg bg-slate-900 px-3 py-1 text-[11px] font-bold text-white hover:bg-slate-800 cursor-pointer">Close</button>
                </div>
            </div>
        </div>
    </div>
