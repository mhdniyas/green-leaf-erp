@php
    ...
    $items = [
        ...
        ['label' => 'Aishwarya Veg', 'href' => route('admin.finance-v2.aishwarya-veg', ['date' => $navDate]), 'active' => request()->routeIs('admin.finance-v2.aishwarya-veg*') || request()->routeIs('admin.finance-v2.shops.*')],
        ['label' => 'Payments', 'href' => route('admin.finance-v2.payments.index', ['date' => $navDate]), 'active' => request()->routeIs('admin.finance-v2.payments.*')],
        ['label' => 'Reports', 'href' => route('admin.finance-v2.reports', ['date' => $navDate]), 'active' => request()->routeIs('admin.finance-v2.reports')],
    ];
@endphp

<div class="flex flex-col gap-3 rounded-[1.5rem] border border-slate-200 bg-white p-3 shadow-sm lg:flex-row lg:items-center lg:justify-between">
    ...
    <form method="GET" action="{{ url()->current() }}" class="flex items-center gap-2">
        <label class="rounded-[1rem] border border-slate-200 bg-slate-50 px-3 py-2">
            <span class="block text-[10px] font-black uppercase tracking-[0.16em] text-slate-400">Month</span>
