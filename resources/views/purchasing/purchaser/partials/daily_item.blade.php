...
<article class="relative min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    <div class="flex min-w-0 items-center gap-2 px-2.5 py-2">
        <button type="button" @unless ($isGradeBCatalog) onclick="toggleDailyItemDetails('{{ $dailyItemId }}')" @endunless class="flex min-w-0 flex-1 items-center gap-2 text-left">
            <div class="min-w-0 flex-1">
                ...
                <div class="mt-1 flex min-w-0 flex-wrap items-center gap-1.5 text-[10px] font-black text-slate-500">
                    ...
                    @else
                        <span class="rounded-full bg-blue-50 px-2 py-0.5 text-blue-700">B order {{ number_format($summary['total_approved_qty'], 2) }} {{ $summary['unit'] }}</span>
        ...
        @if ($isGradeBCatalog || $summary['remaining_qty'] > 0)
            <button type="button" onclick="event.stopPropagation(); openAddToCartModal({{ $summary['product_id'] }}, '{{ addslashes($summary['product_name']) }}', '{{ $summary['unit'] }}', {{ $summary['remaining_qty'] }}, {{ $summary['draft_qty'] }}, '{{ $step }}', '{{ addslashes(implode(', ', $summary['draft_purchasers'])) }}', '{{ $isDirectCatalog ? 'green_leaf_direct_purchase' : 'shop_order' }}')" class="inline-flex h-8 shrink-0 items-center justify-center gap-1 rounded-lg bg-teal-600 px-2.5 text-[11px] font-black text-white shadow-sm transition-all hover:bg-teal-500">
    ...
    <div id="{{ $dailyItemId }}" class="hidden border-t border-slate-100 p-3">
        <div class="flex flex-col gap-4">
            ...
            <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3">
                ...
                @endif
                <button type="button" onclick="openDemandDetailsModal({{ $summary['product_id'] }})" class="mt-3 flex w-full items-center justify-between text-left text-[11px] font-black text-slate-500">
