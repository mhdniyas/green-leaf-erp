            ...
            @if(count($draftItemsList) > 0)
                ...
                <div
                    ...
                    <form action="{{ route('purchaser.purchase.submit') }}" method="POST">
                        ...
                        <button type="submit"
                            class="bg-white hover:bg-slate-50 text-cyan-750 font-black text-xs px-4 py-2.5 rounded-xl transition border-0 cursor-pointer shadow-sm">
                            Submit to Manager
                        </button>
                    </form>
                </div>
                    ...
                    <form action="{{ route('purchaser.purchase.draft.record') }}" method="POST" class="space-y-3">
                        ...
                        <div>
                            ...
                            <select name="supplier_id" required
                                class="w-full text-xs font-bold bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2.5 text-slate-800 dark:text-white focus:outline-none focus:ring-1 focus:ring-cyan-500 cursor-pointer">
                                <option value="">Select Supplier</option>
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}">{{ $supplier->name }} ({{ $supplier->category }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
