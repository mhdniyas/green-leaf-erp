<x-layouts.app title="Create Purchase Order">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden max-w-4xl">
        <div class="px-6 py-5 border-b border-gray-100">
            <h2 class="text-sm font-semibold text-gray-900">New Purchase Order</h2>
            <p class="text-xs text-gray-500 mt-0.5">Create a draft purchase order for a supplier.</p>
        </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="supplier_id" name="supplier_id" required
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" @selected(old('supplier_id') == $supplier->id)>{{ $supplier->name }} ({{ $supplier->type }})</option>
                        @endforeach
                    </select>
