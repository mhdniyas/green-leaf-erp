        ...
        <div class="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            @foreach ($section['users'] as $demoUser)
                <form method="POST" action="{{ route('login.demo', $demoUser) }}">
                    ...
                    <button
                        type="submit"
                        class="flex h-full min-h-24 w-full flex-col justify-between gap-3 rounded-xl border border-emerald-100 bg-white px-3 py-3 text-left transition hover:border-emerald-300 hover:bg-emerald-50 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                    >
                        ...
                        <span class="flex max-w-full flex-wrap gap-1">
