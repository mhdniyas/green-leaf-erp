...
class OtherExpenseController extends Controller
{
    public function __construct(
        private readonly OtherExpensePostingService $postingService,
    ...
    public function index(Request $request): View
    {
        ...
        $editingExpense = $request->filled('edit')
            ? OtherExpense::query()->whereKey($request->integer('edit'))->where('user_id', $request->user()->id)->first()
            : null;
        ...
        $selectedDateExpenses = $expenses
            ->filter(fn (OtherExpense $expense): bool => $expense->expense_date?->isSameDay($date) === true)
            ->values();

        return view('purchasing.purchaser.other_expenses', [
            'date' => $date,
            'previousDate' => $date->copy()->subDay()->toDateString(),
            'nextDate' => $date->copy()->addDay()->toDateString(),
            'categories' => OtherExpense::categories(),
            'expenses' => $expenses,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $expense = DB::transaction(function () use ($request, $validated): OtherExpense {
            $expense = OtherExpense::query()->create([
        ...
        return redirect()
            ->route('purchaser.other-expenses.index', ['date' => $expense->expense_date->toDateString()])
            ->with('success', 'Other expense saved and posted to daily company expense.');
    }

    public function update(Request $request, OtherExpense $expense): RedirectResponse
    {
        $this->ensurePurchaser($request);
        ...
        return redirect()
            ->route('purchaser.other-expenses.index', ['date' => $newDate->toDateString()])
            ->with('success', 'Other expense updated and daily company expense resynced.');
    }

    public function destroy(Request $request, OtherExpense $expense): RedirectResponse
    {
        $this->ensurePurchaser($request);
        $this->ensureExpenseOwner($request, $expense);
        ...
        return redirect()
            ->route('purchaser.other-expenses.index', ['date' => $date->toDateString()])
            ->with('success', 'Other expense deleted and daily company expense resynced.');
    }
    ...
    private function ensureExpenseOwner(Request $request, OtherExpense $expense): void
    {
