    ...
    public function show(ShopOrder $shopOrder, Request $request): View
    {
        ...
        $productGroups = $shopOrder->items
            ...
            ->map(function ($items) {
                ...
                // Available = running ledger balance + what has already been loaded for this order
                $available = max(0.0, $this->stockLedgerService->availableStockForProduct($productId) + $totalLoaded);
        ...
        try {
            DB::transaction(function () use ($shopOrder, $request, $userId) {
                ...
                foreach ($request->input('items', []) as $productId => $submittedQty) {
                    ...
                    if ($diff > 0.001) {
                        // Check available stock using running ledger balance
                        $availableStock = $this->stockLedgerService->availableStockForProduct($productId);
