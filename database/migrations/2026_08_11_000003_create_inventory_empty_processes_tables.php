...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('inventory_empty_process_items', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('process_id')->constrained('inventory_empty_processes')->cascadeOnDelete();
            $table->foreignId('product_id')->constrained()->restrictOnDelete();
            $table->string('status', 20)->default('pending');
            $table->text('error_message')->nullable();
            $table->timestamps();
            $table->unique(['process_id', 'product_id']);
    ...
    {
        Schema::dropIfExists('inventory_empty_process_items');
        Schema::dropIfExists('inventory_empty_processes');
    }
};
