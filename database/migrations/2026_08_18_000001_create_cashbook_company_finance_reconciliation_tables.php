...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('cashbook_company_account_statement_entries', function (Blueprint $table): void {
            $table->id();
            $table->foreignId('company_account_id')->constrained('cashbook_company_accounts')->cascadeOnDelete();
            ...
            $table->string('statement_batch', 120)->nullable()->index();
            ...
            $table->index(['company_account_id', 'transaction_date']);
        ...
        Schema::create('cashbook_company_payment_reconciliations', function (Blueprint $table): void {
            ...
            $table->foreignId('payment_request_id')
                ->constrained('shop_invoice_payment_requests')
                ->cascadeOnDelete();
            $table->foreignId('shop_id')->constrained('shops')->cascadeOnDelete();
            $table->foreignId('company_account_id')->constrained('cashbook_company_accounts')->cascadeOnDelete();
            $table->foreignId('statement_entry_id')
                ->nullable()
                ->constrained('cashbook_company_account_statement_entries', indexName: 'cashbook_recon_statement_entry_fk')
                ->nullOnDelete();
            ...
            $table->foreignId('difference_entry_type_id')->nullable()->constrained('ledger_entry_types')->nullOnDelete();
            $table->foreignId('difference_transaction_id')
                ->nullable()
                ->constrained('shop_ledger_transactions')
                ->nullOnDelete();
        ...
        Schema::table('shop_invoice_payment_requests', function (Blueprint $table): void {
            if (! Schema::hasColumn('shop_invoice_payment_requests', 'reconciliation_status')) {
                $table->string('reconciliation_status', 30)->default('pending')->after('status')->index();
            }
